...
Crawler.prototype.addNodes = function(nodes) {
  let nodeObjects = nodes.map(function(node) {
    if (typeof node === 'string') {
      return {type: 'ledger', uri: node}
    } else if (typeof node === 'object' && typeof object.type === 'string') {
      return node
    } else {
      throw new Error('Invalid node: ' + JSON.stringify(node))
    }
  })
  this.queue = this.queue.concat(nodeObjects)
